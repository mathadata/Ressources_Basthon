...
def import_js_scripts():
    run_js("""
        ...
        require(['ag-grid-community', 'chartjs'], function(agGrid) {
...
def calculer_score(algorithme, method=None, parameters=None, cb=None, a=None, b=None, caracteristique=None):
    try:
        ...
        print("Pourcentage d'erreur : " + score_str(score))
        # set_score(score)
...
run_js("""
    ...
    const mathadata = {
        ...
        exercice_droite_carac(id, params) {
            ...
            chart.data.datasets[1].data = new Array(i_exercice_droite_carac).fill(0).map((_, index) => ({x: c_train[index], y: 0}))
            ...
            if (i_exercice_droite_carac < c_train.length) {
                ...
                chart.options.onClick = (e) => {
                    ...
                    if (Math.abs(dataX - c_train[i_exercice_droite_carac]) < 1) {
                        chart.data.datasets[1].data.push({x: c_train[i_exercice_droite_carac], y: 0});
                ...
                data: {
                    ...
                    },
                    {
                        ...
                        pointBackgroundColor: function(context) {
                            ...
                            return window.mathadata.classColors[labels[index]];
                        },
                    },
                    ]
                },
