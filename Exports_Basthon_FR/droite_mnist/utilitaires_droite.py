...
def tracer_points_droite(id=None, input="range", carac=None):
    if id is None:
        id = uuid.uuid4().hex
    display(HTML(f'''
...
def affichage_zones_custom(A1, B1, A2, B2):
    ...
    tracer_points_droite(input="number", carac=common.challenge.deux_caracteristiques_custom)
...
def afficher_customisation():
    ...
    tracer_points_droite(id=id, input="number", carac=common.challenge.deux_caracteristiques_custom)
...
validation_question_score_droite = MathadataValidateVariables({
    ...
    success="C'est la bonne réponse. Il y a un point bleu sous la droite et un point orange au dessus soit deux erreurs donc 20%.",
    on_success=lambda answers: set_step(2)
)
validation_execution_tracer_points_droite = MathadataValidate(success="")
validation_score_droite = MathadataValidate(success="Bravo, vous pouvez passer à la suite.")
