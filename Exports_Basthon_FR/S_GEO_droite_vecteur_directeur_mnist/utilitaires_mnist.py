...
strings = {
    ...
    "objectif_score_droite_vecteur_normal": 10,
...
with open(d_train_path, 'rb') as f:
    ...
    if len(d_train_full) > 0:
        split_index = int(5/6 * len(d_train_full))
...
with open(r_train_path, 'rb') as f:
    ...
    if len(r_train_full) > 0:
        split_index = int(5/6 * len(r_train_full))
...
class Mnist(common.Challenge):
    def __init__(self):
        ...
        self.objectif_score_droite_vecteur_normal = strings['objectif_score_droite_vecteur_normal']
    ...
    def deux_caracteristiques_moy_haut_moy_bas(self, d):
        # on retourne les coordonnées (k1, k2) du point dans le plan défini par ces deux zones haut et zone basse
        
        k1 = moyenne_zone(d, (0, 0), (13, 27))  # top half
        k2 = moyenne_zone(d, (14, 0), (27, 27))  # bottom half
        return (k1, k2)
...
window.mathadata.interface_data_gen = (id) => {
    ...
    if (container._drawingStopSelecting) {
        document.removeEventListener('mouseup', container._drawingStopSelecting);
        window.removeEventListener('blur', container._drawingStopSelecting);
    }
    container._drawingStopSelecting = stopSelecting;
    document.addEventListener('mouseup', container._drawingStopSelecting);
    window.addEventListener('blur', container._drawingStopSelecting);
    ...
    table.addEventListener('mouseup', stopSelecting);
...
(function () {
    ...
    // ---------------------------------------------------------------------
        ...
        try {
	        ...
	        const roundPointValues = Boolean(params?.round_point_values);
        ...
        const escapeHtml = (s) => String(s)
	        ...
	        if (showPoint && zonesAdj && zonesAdj.length >= 2) {
	            ...
	            thresholded.forEach((tImg, i) => {
	                let x = window.mathadata.mnist_mean_zone(tImg, zonesAdj[0]);
	                let y = window.mathadata.mnist_mean_zone(tImg, zonesAdj[1]);
	                const formatPt = roundPointValues ? (v) => String(Math.round(v)) : format1;

	                const pt = document.getElementById(`${id}-pt-${i}`);
	                if (!pt) return;

	                const name = escapeHtml(pointNames[i] || defaultNames[i] || `P${i + 1}`);
	                pt.innerHTML = `${name}(<span class="mathadata-carac-x">${escapeHtml(formatPt(x))}</span>&nbsp;;&nbsp;<span class="mathadata-carac-y">${escapeHtml(formatPt(y))}</span>)`;
	            });
