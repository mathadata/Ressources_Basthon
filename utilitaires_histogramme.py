...
def compute_histogramme(feature):
    c_train = compute_c_train(feature, common.challenge.d_train)
    data = {}
...
def display_histogram(div_id=None, seuil=None, feature=None, legend=False):
    if div_id is None:
        div_id = uuid.uuid4().hex
        display(HTML(f'<canvas id="{div_id}"></canvas>'))
        
    if not feature:
        feature = common.challenge.feature
        
    data = compute_histogramme(feature)
...
def compute_score_hist_thresold():
    ...
    r_low_feature = common.challenge.r_low_feature
    r_high_feature = common.challenge.r_high_feature
    feature = common.challenge.feature
...
window.mathadata.displayHisto = (div_id, data, with_legend, seuil, with_axes_legend) => {
    ...
    const config = {
        ...
        data: {
            datasets: [
                {
                    label: `Count of images of ${with_legend ? 2 : '?'}`,
                    data: data_1,
                ...
                {
                    label: `Count of images of ${with_legend ? 7 : '?'}`,
                    data: data_2,
