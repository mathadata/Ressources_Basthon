...
def plot_points_vector(id=None, carac=None):
    if id is None:
        id = uuid.uuid4().hex
    display(HTML(f'''
        <div style="text-align: center; font-weight: bold; font-size: 2rem;">Error: <span id="{id}-score">...</span></div>
        ...
        <div style="display: flex; gap: 1rem; justify-content: center; flex-direction: row;">
            <div>
                ...
                <input type="number" value="5" step="1" id="{id}-input-a">
            </div>
            <div>
                <label for="{id}-input-b" id="{id}-label-b"></label>
                <input type="number" value="10" step="1" id="{id}-input-b">
            </div>
            <div>
                <label for="{id}-input-y" id="{id}-label-y"></label>
                <input type="number" value="10" step="1" id="{id}-input-y">
            </div>
        </div>             
    '''))
    ...
    run_js(f"setTimeout(() => window.mathadata.plot_points_vector('{id}', '{json.dumps(params, cls=NpEncoder)}'), 100)")

def scalar_product_exercise():
...
def display_custom_zones(A1, B1, A2, B2):
    ...
    plot_points_vector(carac=common.challenge.deux_caracteristiques_custom)
...
def display_customization():
    ...
    plot_points_vector(id=id, carac=common.challenge.deux_caracteristiques_custom)
    ...
    run_js(f'''
        window.mathadata.on_custom_update = () => {{
            window.mathadata.run_python('update_custom()', (points) => {{
                ...
                mathadata.plot_points_vector('{id}', params)
            }})
        }}
    ''')
...
run_js('''
    ...
    window.mathadata.plot_points_vector = function(id, params) {
        mathadata.tracer_points(`${id}-chart`, params);
        if (typeof params === 'string') {
            params = JSON.parse(params);
        }
        ...
        const update = () => {
            ...
            const y = parseFloat(input_y.value);
            ...
            const m = b/a
            const p = y
            const lineData = [{x: min_x, y: m * min_x + p}, {x: max_x, y: m * max_x + p}]
            chart.data.datasets[3].data = lineData
            const c = a * y - b * 0
            chart.data.datasets[3].label = `${b}x ${a <= 0 ? '+' : '-'} ${Math.abs(a)}y ${c < 0 ? '-' : '+'} ${Math.abs(c)} = 0`
            ...
            exec = setTimeout(() => {
                ...
                mathadata.run_python(`set_vector_parameters(${a}, ${b}, ${y})`)
            }, 200)

            label_a.innerHTML = `a = ${displayValue ? a : ''}`
            label_b.innerHTML = `b = ${displayValue ? b : ''}`
            label_y.innerHTML = `y = ${displayValue ? y : ''}`
        }
        ...
        input_y = removeAllEventListeners(input_y);

        input_a.addEventListener("input", update);
        input_b.addEventListener("input", update);
        input_y.addEventListener("input", update);
        update()
    }
''')
...
b = None
y = None
def set_vector_parameters(a_val, b_val, y_val):
    global a, b, y
    a = a_val
    b = b_val
    y = y_val
...
def compute_score_custom_line():
    ...
    def cb(score):
        if score < 0.06:
            ...
            print_error("Keep looking for 2 zones to have less than 6% error. Remember to update the values of a, b and y after defining your zone.")
...
def function_validation_normal(errors, answers):
    ...
    if not (isinstance(n, tuple) and len(n) == 2 and all(isinstance(x, (int, float)) for x in n)):
        errors.append("Write the vector coordinates between parenthesis separated by a comma. For non integer values, use a dot. Example: (3.5 , 5)")
        return False

    if n[0]*a+n[1]*b!=0:
        errors.append("This isn't a correct answer. A vector n orthogonal  to an other vector u (a, b) verify n.u=0. A possibility is the vector (-b,a).")
        return False
...
validation_execution_scalar_product_exercise = MathadataValidate(success="")
validation_question_scalar_product = MathadataValidateVariables({
    'scalar_product': -40
})
validation_question_scalar_product_2 = MathadataValidateVariables({
    'scalar_product': 10
}, tips=[
    {
      'trials': 1,
      'tip': 'You have to calculate the scalar product between n(-2, 4) and AM(?, ?) with A(20, 30) and M(25, 35)'
    },
    {
      'trials': 2,
      'seconds': 30,
      'tip': 'The vector AM coordinates are (5, 5) (given by (25-20, 35-30))'
    },
    {
      'trials': 3,
      'seconds': 60,
      'tip': 'To calculate the scalar product, you have to multiply the coordinates of the two vectors and add the results. We have n(-2, 4) and AM(5, 5).'
    },
    {
      'trials': 4,
      'seconds': 120,
      'tip': 'The scalar product is given by -2*5 + 4*5 = 10'
    }
  ])
